...
def mark_contact_as_read(contact_id: int) -> None:
    db = get_db()
    db.execute("UPDATE contact SET status = 'read' WHERE id = ?", (contact_id,))
    db.commit()


def delete_contact(contact_id: int) -> bool:
    db = get_db()
    cursor = db.execute("DELETE FROM contact WHERE id = ?", (contact_id,))
    db.commit()
...
def get_all_orders_admin(status_filter: str = "all") -> list[sqlite3.Row]:
    ...
    return get_db().execute(
        ...
        SELECT o.id, o.user_id, o.total_amount, o.status, o.created_at,
               u.email, COUNT(oa.id) AS items_count
        FROM orders AS o
...
def get_order_items_by_order_id(order_id: int) -> list[sqlite3.Row]:
    return get_db().execute(
        """
        SELECT oa.id, oa.order_id, oa.article_id, oa.quantity, oa.unit_price,
               a.name, a.image
...
def update_article(article_id: int, data: dict[str, Any]) -> bool:
    ...
    cursor = db.execute(
        """
        UPDATE articles
        SET name = ?, genres = ?, universe = ?, image = ?, price = ?,
            stock = ?, release_day = ?
        WHERE id = ?
        """,
        (
